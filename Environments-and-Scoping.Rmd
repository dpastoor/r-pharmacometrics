---
title: "Environment and Scoping Rules"
author: "Devin"
...
ourDF <- data.frame(ID = 1:3)
ourDF <- dosing_col(ourDF, dose = 500)
ourDF
```
...
**Environments** are R objects that contain the sets of symbols available in a given context, the objects associated with these symbols, and a *pointer* to the parent environment. In laymans terms, an environment is a place to store both objects themselves, and how they are related. The overall purpose of an environment is to bind a set of names to values. Every environment has a parent environment (expect for one env - the *empty environment*), and can also have multiple "child" environments. 
#### TODO: reword environment definitions
...
it assigns the symbol `DOSE` to a vector object of length one with a value of `100` in the global environment.
...
One situation where the order of the search list comes into play is when two packages both have a function with the same name. 
...
We have a function `f()` that simply prints the value stored in the variable dose.
